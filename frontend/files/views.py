...
import requests
from my_storage_gui.settings import STORAGE_API_URL
import json
...
def download(request, path):
    try:
        ...
        if raw:
            return redirect(f'{STORAGE_API_URL}/upload{raw}&path={path}')
        dir_tree = requests.get(f'{STORAGE_API_URL}/dir').json()
